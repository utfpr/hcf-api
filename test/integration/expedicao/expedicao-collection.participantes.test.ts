  ...
  describe('Substituir Rotas', () => {
    test('substituteRoute remove as antigas e insere as novas na ordem correta', async () => {
      ...
      try {
        ...
        const rotasNoBanco = await knex<{ expedicao_id: number; ordem: number; cidade_id: number | string }>('expedicoes_rotas')
          .where({ expedicao_id: expedicaoId })
          .orderBy('ordem')

        expect(rotasNoBanco.length).toBe(3)

        const arrayValidacao = rotasNoBanco.map((rota: { ordem: number; cidade_id: number | string }) => [
          rota.ordem,

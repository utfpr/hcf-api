...
export class RequerAcessoEscritaFaseSucessional implements RequestHandler {
  async handle(request: HttpRequest, next: NextHandler): Promise<HttpResponse | HttpError> {
    ...
    try {
      ...
      if (!Number.isInteger(tipoUsuarioId) || !ALLOWED_TIPOS_USUARIOS.has(tipoUsuarioId)) {
        return new ForbiddenError({ message: 'Usuário sem permissão para alterar fases sucessionais' })
      }

...
const obterModeloDarwinCoreLotes = async (limit, offset, request, response) => {
  ...
  tombos.forEach(tombo => {
      ...
      const family = tombo.familia?.nome === 'Indeterminada' ? '' : tombo.familia?.nome || '';
      const genus = tombo.especie?.nome || '';
      const species = tombo.especie_especie_2 || '';
      const scientificNameAuthor = tombo.especie_especie_autor || '';
      const commonName = tombo.nomes_populares || '';
      const catalogNumber = tombo.hcf || '';
      const basisOfRecord = 'PreservedSpecimen';
      const typeStatus = tombo.tipo?.descricao || '';
      ...
      const country = tombo.local_coletum?.pais || '';
      const stateOrProvince = tombo.local_coletum?.estado || '';
      const city = tombo.local_coletum?.cidade || '';
      const locality = tombo.local_coletum?.local || '';
      ...
      const identifierName = tombo.identificadores
          ...
          : '';
      const notes = tombo.retirada_exsiccata_tombos?.cod_barra
          ? `[BARCODE=${tombo.retirada_exsiccata_tombos.cod_barra}] ${tombo.observacao || ''}`
          : tombo.observacao || '';
      ...
      response.write(`${linha.replace(/null|undefined/g, '')}\n`);
  });
};
